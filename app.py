    ...
    if st.button("🔍 계약서 분석 시작"):
        with st.spinner("분석 중입니다..."):
            ...
            if st.session_state.analysis_done and st.session_state.analysis_result:
                ...
                st.text_area("분석된 계약서 텍스트", value=result["text"], height=300)
...
if st.session_state.analysis_done and st.session_state.analysis_result:
    ...
    text = "임대인은 계약 종료 시 보증금을 반환하지 않을 수 있다."

    # ✅ 사기 여부 판별 (FAISS)
    is_fraud, top_docs = search_and_judge_fraud(text, faiss_path="faiss_index")

    if is_fraud:
        st.markdown("🔥 **판단 결과: 사기 계약 가능성이 있습니다.**")
        st.error("❗ 사기 계약서와 유사한 내용이 포함되어 있습니다.")
    else:
        st.markdown("🟢 **판단 결과: 정상 계약으로 간주됩니다.**")
        st.success("✅ 사기 계약서와 유사한 위험 요소가 발견되지 않았습니다.")

    # 🔍 유사 문장 보기
    with st.expander("🔍 유사한 사기 계약 문장 보기"):
